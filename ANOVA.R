...
# only two cars have 6 carborators and 8 carborators each, It may affect the model so we delete those two observation from the data
...
ggplot(data = df1,aes(sample=mpg))+
  stat_qq()+
  stat_qq_line()'''
shapiro.test(df1$mpg)  #accept null hypothesis 'mpg' is normal because p-value is greater than 0.05
